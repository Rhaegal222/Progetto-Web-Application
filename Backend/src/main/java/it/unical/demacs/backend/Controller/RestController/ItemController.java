...
public class ItemController {
    ...
    public ResponseEntity<?> deleteItem(@RequestParam long idItem){return itemService.deleteItem(idItem);}
    ...
    public ResponseEntity<?> searchItems(@RequestParam(required = false) String search, @RequestParam(required = false) String category) {
        if (search == null && (category == null || category.equals("all"))) {return itemService.allItems();}
        else if (search == null) {return itemService.searchItems("", category);}
        else if (category == null || category.equals("all")) {return itemService.searchItems(search, "");}
        else {return itemService.searchItems(search, category);}
    }
